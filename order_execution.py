...
class OrderExecutor:
    ...
    async def open_position(self, symbol: str, side: str, size: float,                   # trading_strategy부분에서 롱 진입 생성 숏 진입 생성 후에 호출하는 함수다.
        ...
        try:
            ...
            # 레버리지 설정
            await self._set_position_leverage(symbol, leverage)  #이거 이상함. 그냥 class에서 받아와야하는게 맞는거같은데. 일단 구현완료. 인터페이스 처럼 구현함.
            
            # API 파라미터 설정
            api_side = 'buy' if side == 'long' else 'sell'
            hold_side = side  # long 또는 short

             # 문자열로 변환된 값들 준비 JSON 형태때문이다.
            str_size = str(size)
            str_stop_loss = str(round(float(stop_loss_price) * 10) / 10)  # 소수점 첫째자리까지
            ...
            if response.get('code') == '00000':
                ...
                # 리밋 주문인 경우 체결 대기
                if order_type == 'limit':
                    order_filled = await self.wait_for_order_fill(symbol, order_id) # 함수호출.
                    if not order_filled:
                        logger.error("Order not filled within timeout")
                        await self.api.cancel_order(symbol, order_id) # false값을 wait_for_에서 받아서 실행.
                        return False
                else:
                    # 시장가 주문인 경우 짧은 대기
    ...
    async def execute_limit_close(self, position: Position, limit_price: float) -> bool:                # 트레이딩로직에서 호출당하는 함수. 익절주문이 들어갈때 호출. close_position부분.
        """지정가로 포지션 청산"""
        try:
